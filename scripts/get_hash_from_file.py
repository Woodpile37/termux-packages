...
def get_Packages_hash_from_Release(Release_file, arch, component, hash_type="SHA256"):
    ...
    for j in range(i, len(hash_list)):
        if string_to_find in hash_list[j].strip(
            " "
        ) and string_to_find + "." not in hash_list[j].strip(" "):
            hash_entry = list(
                filter(lambda s: s != "", hash_list[j].strip("").split(" "))
            )
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        sys.exit(
            "Too few arguments, I need the path to a Packages file, a package name and a version, or an InRelease file, an architecture and a component name. Exiting"
        )
    ...
    if sys.argv[1].endswith("Packages"):
        ...
        sys.exit(
            sys.argv[1]
            + " does not seem to be a path to a Packages or InRelease/Release file"
        )
