...
class TermuxPackage(object):
    ...
    def __init__(self, dir_path, fast_build_mode):
        ...
        if not os.path.isfile(build_sh_path):
            raise Exception(
                "build.sh not found for package '" + self.name + "'")
        ...
        if not fast_build_mode:
            self.deps.difference_update(
                [subpkg.name for subpkg in self.subpkgs])

        # Populated outside constructor, reverse of deps.
        self.needed_by = set()
...
class TermuxSubPackage:
    ...
    def __init__(self, subpackage_file_path, parent, virtual=False):
        ...
        self.name = os.path.basename(
            subpackage_file_path).split(".subpackage.sh")[0]
        self.parent = parent
...
def generate_target_buildorder(target_path, pkgs_map, fast_build_mode):
    ...
    # Do not depend on any sub package
    if fast_build_mode:
        package.deps.difference_update(
            [subpkg.name for subpkg in package.subpkgs])
...
def main():
    ...
    pkgs_map = read_packages_from_directories(
        packages_directories, fast_build_mode)

    if full_buildorder:
        build_order = generate_full_buildorder(pkgs_map)
    else:
        build_order = generate_target_buildorder(
            package, pkgs_map, fast_build_mode)
